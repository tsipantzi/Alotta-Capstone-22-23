...
public class CustomerInventoryServiceImpl implements CustomerInventoryService {
    ...
    @Override
    public CustomerInventory saveCouponForCustomer(Long userId, Long couponId) {
        final var user = appUserDao.findById(userId)
            .orElseThrow(() -> new IllegalArgumentException(String.format("app user %s does not exist ", userId)));

        final var foundCoupon = couponDao.findById(couponId)
            .orElseThrow(() -> new IllegalArgumentException(String.format("Coupon %s does not exist", couponId)));

        return customerInventoryDao.saveAndFlush(
            new CustomerInventoryEntity(foundCoupon, user)
        ).toDomainObject();
    }
